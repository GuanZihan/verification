...
%     Q11 = -2*diag(alpha_param.*beta_param)*(diag(lambda));
%     Q12 = diag(alpha_param+beta_param)*(diag(lambda))+T;
%     Q13 = -nu+ delta * (X_min - X_max);
%     Q22 = -2*diag(lambda)-2*D-2*T;
%     Q23 = nu+eta+D *(X_min+X_max) + original_beta' * delta * X_max ;
%     Q33 = -2*X_min'*D*X_max - 2 * X_min'*delta*X_max;
    
%     Q11 = -2*diag(alpha_param.*beta_param)*(diag(lambda));
%     Q12 = diag(alpha_param+beta_param)*(diag(lambda))+T;
%     Q13 = -nu+ delta .* Y_max;
%     Q22 = -2*diag(lambda)-2*D-2*T;
%     Q23 = nu+eta+D *(X_min+X_max) + (Y_min - Y_max) .* delta;
%     Q33 = -2*X_min'*D*X_max - 2 * (Y_min.*Y_max)' * delta;

%     Q11 = -2*diag(alpha_param.*beta_param)*(diag(lambda));
%     Q12 = diag(alpha_param+beta_param)*(diag(lambda))+T;
%     Q13 = -nu + delta * (original_beta.* Y_max);
%     Q22 = -2*diag(lambda)-2*D-2*T;
